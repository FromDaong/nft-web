...
const getListingsByTag = (tag: string, page: number) => {
    return axios.get(`${apiEndpoint}/marketplace/nft/listings-by-tag?tag=${tag}&page=${page}`).then(res => res.data.data)
}
...
export default function Address(props) {
    ...
    const {
        ...
        queryKey: [`sweetshopNFTsInfiniteByTag:${tag}`],
        queryFn: ({pageParam = 1}) => getListingsByTag(tag, pageParam),
        ...
        <ApplicationLayout>
            <ApplicationFrame>
                ...
                <Container className={"flex py-12"}>
                    <Heading size={"xs"}>
                        Showing NFT's with the tag ${tag}
                    </Heading>
                </Container>
