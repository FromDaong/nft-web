...
import { getTreatMarketplaceAddress, getTreatNFTMinterContract } from '../treat/utils'
...
const useGetMinterIsApprovedForAll = (tokenAddress: string) => {
  ...
  const fetchAllowance = useCallback(async () => {
    if(treat) {
      const _allowance = await getIsApprovedForAll(treatNftMinterContract, treatMarketplaceAddress, account)
