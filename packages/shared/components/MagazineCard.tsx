...
export default function MagazineCard(mag: Magazine) {
	...
	useEffect(() => {
		if (colorMap.length > 0) {
			setLinearBackground(
				`linear-gradient(0deg, rgb(${hexToRgb(
					colorMap[0]
				)}/95) 0%, rgb(${hexToRgb(colorMap[1])} / 95%) 35%, rgb(${hexToRgb(
					colorMap[1]
				)} / 63%) 50%, rgb(${hexToRgb(
					colorMap[1]
				)} / 11%) 78%, rgb(0 0 0 / 91%) 100%)`
			);
		}
	}, [colorMap]);
	...
	return (
		<>
			...
			<div
				className="relative col-span-3 h-[480px] border shadow-xl overflow-clip rounded-xl md:col-span-2 lg:col-span-1"
				key={mag.cover}
			>
				<Link href={mag.href}>
					<a
