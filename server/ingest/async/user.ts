...
export const createStepFnForRefferals = () => createStepFunction<Added>(
  "user/email.added",
  // @ts-ignore
  ({ event, tools }) => {
    const newUserJoinedWithReferral = tools.waitForEvent("user/profile.created", {
      timeout: "24h",
    ...
    tools.run("Send reminder", () => {
      ...
      sendReminderToReferFriends({ email: event.user.email, cart: event.data.cart_id });
    });
  },
  {
    cancellation: {
