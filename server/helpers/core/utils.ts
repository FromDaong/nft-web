import {MongoModelCreator, MongoModelTransaction} from "../models/index";
...
export const populateNFTsWithProfileAndTx = async (nfts, soldOut = true) => {
	return Promise.all(
		nfts
			.map(async (nft) => {
				...
				if (soldOut && mints.length === Number(nft.max_supply)) {
					return null;
				}
